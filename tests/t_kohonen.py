...
def draw2(X, map_length, iter, Thetas, learning_rate, sigma,  neighbour_change=-1, **kwargs):
    if iter % 10 != 0:
        return
    plt.figure(1).clear()
    plt.title(
...
def kohonen_square_10():
    km = kh.KohonenMap(10, init_learning_rate=1, learning_rate_constant=3000, init_sigma=2.5, sigma_constant=3000)
    rng = np.random.default_rng()
    X = rng.uniform(size=(1000, 2))
...
def kohonen_square_20():
    km = kh.KohonenMap(20, init_learning_rate=1, learning_rate_constant=12000, init_sigma=5, sigma_constant=12000)
    rng = np.random.default_rng()
...
def kohonen_square_regions():
    km = kh.KohonenMap(20, init_learning_rate=1, learning_rate_constant=10000, init_sigma=4, sigma_constant=5000, observer=draw2)
    rng = np.random.default_rng()
